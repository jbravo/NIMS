...
sub_stack-show-all() {
    docker-compose -p nims \
      ...
      -f deploy/docker-compose-services-stack-dev-remote.yml \
      ps
    docker-compose -p nims -f deploy/docker-compose-manager.yml ps
}
...
sub_service-stack-restart-service(){
    ...
    echo "Restarting $1"
    echo "Stopping $1..."
    docker-compose -p nims -f deploy/docker-compose-services-stack.yml -f deploy/docker-compose-services-stack-dev-remote.yml stop $1
    ...
    docker-compose -p nims -f deploy/docker-compose-services-stack.yml -f deploy/docker-compose-services-stack-dev-remote.yml build $1
    echo "Starting $1 container..."
    docker-compose -p nims -f deploy/docker-compose-services-stack.yml -f deploy/docker-compose-services-stack-dev-remote.yml up -d $1
...
sub_mgr-up() {
    docker-compose -p nims -f deploy/docker-compose-manager.yml up -d
    ip=`ifconfig eth0 | sed -En 's/127.0.0.1//;s/.*inet (addr:)?(([0-9]*\.){3}[0-9]*).*/\2/p'`
    echo "DONE! Access http://$ip for URL list down page"
}

sub_mgr-down() {
    docker-compose -p nims -f deploy/docker-compose-manager.yml down -v
}

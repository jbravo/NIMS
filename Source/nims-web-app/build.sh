...
SERVICE_STACK_COMPOSE_FILES_DEFAULT_OPTS="-f deploy/service/docker-compose-services-stack.yml"
SERVICE_STACK_COMPOSE_FILES_STAGING_OPTS="-f deploy/service/docker-compose-services-stack.yml -f deploy/service/docker-compose-services-stack-staging.yml"
SERVICE_STACK_COMPOSE_FILES_PRE_PRODUCTION_OPTS="-f deploy/service/docker-compose-services-stack.yml -f deploy/service/docker-compose-services-stack-pre-production.yml"
SERVICE_STACK_COMPOSE_FILES_PRODUCTION_OPTS="-f deploy/service/docker-compose-nims-web.yml"
...
sub_coverage() {
    ...
    _docker_mvn test jacoco:report-aggregate
    echo "DONE!"
}
...
sub_docker-build() {
    ...
    docker-compose -f deploy/service/docker-compose-services-stack.yml build
}
...
sub_staging-docker-build() {
    ...
    docker-compose -p nims\
      -f deploy/service/docker-compose-services-stack.yml \
      -f deploy/service/docker-compose-services-stack-staging.yml \
      build
}

get_service_stack_compose_file_opts(){
    SERVICE_STACK_COMPOSE_FILE_OPTS=$SERVICE_STACK_COMPOSE_FILES_DEFAULT_OPTS
    if [[ -n $1 && $1 == "--staging" ]]; then
      SERVICE_STACK_COMPOSE_FILE_OPTS=$SERVICE_STACK_COMPOSE_FILES_STAGING_OPTS
    elif [[ -n $1 && $1 == "--pre-production" ]]; then
      SERVICE_STACK_COMPOSE_FILE_OPTS=$SERVICE_STACK_COMPOSE_FILES_PRE_PRODUCTION_OPTS
    elif [[ -n $1 && $1 == "--production" ]]; then
      SERVICE_STACK_COMPOSE_FILE_OPTS=$SERVICE_STACK_COMPOSE_FILES_PRODUCTION_OPTS
    fi
    echo $SERVICE_STACK_COMPOSE_FILE_OPTS
}


sub_service-stack-build() {
    echo "Need to package before building with docker-compose.."
#    sub_package
    docker-compose -p nims $(get_service_stack_compose_file_opts $1) build
}

sub_service-stack-pull(){
    docker-compose -p nims \
      -f deploy/service/docker-compose-services-stack.yml \
      -f deploy/service/docker-compose-services-stack-dev-remote.yml \
      pull
}
...
sub_stack-show-all() {
    docker-compose -p nims \
      ...
      -f deploy/service/docker-compose-services-stack-dev-remote.yml \
      ps
    docker-compose -p nims -f deploy/service/docker-compose-manager.yml ps
}
...
sub_service-stack-restart-service(){
    ...
    echo "Restarting $1"
    echo "Stopping $1..."
    docker-compose -p nims -f deploy/service/docker-compose-services-stack.yml -f deploy/service/docker-compose-services-stack-dev-remote.yml stop $1
    ...
    docker-compose -p nims -f deploy/service/docker-compose-services-stack.yml -f deploy/service/docker-compose-services-stack-dev-remote.yml build $1
    echo "Starting $1 container..."
    docker-compose -p nims -f deploy/service/docker-compose-services-stack.yml -f deploy/service/docker-compose-services-stack-dev-remote.yml up -d $1
    echo "Wait for $1 port is up..."

    NEXT_WAIT_TIME=0
    CONTAINER_HEALTH_CHECK=$(docker inspect -f '{{json .State.Health.Status}}' nims-$1 | tr -d \" )
...
sub_mgr-up() {
    docker-compose -p nims -f deploy/service/docker-compose-manager.yml up -d
    ip=`ifconfig eth0 | sed -En 's/127.0.0.1//;s/.*inet (addr:)?(([0-9]*\.){3}[0-9]*).*/\2/p'`
    echo "DONE! Access http://$ip for URL list down page"
}

sub_mgr-down() {
    docker-compose -p nims -f deploy/service/docker-compose-manager.yml down -v
}
...
sub_setup() {
    ...
    sudo cp deploy/service/docker/local.repo /etc/yum.repos.d/
    sudo yum update -y
    ...
    sudo cp deploy/service/docker/daemon.json /etc/docker/
    ...
    sudo cp deploy/service/docker/pip.conf /root/.config/pip/

    echo "Install Docker-compose via pip.."
    sudo pip install docker-compose

    echo "Configuring sysctl..."
    sudo cp deploy/service/docker/sysctl.conf /etc/
    sudo sysctl --load=/etc/sysctl.conf

    echo "Configuring system limits..."
    sudo cp deploy/service/docker/limits.conf /etc/security/
}
...
_docker_newman() {
    ...
    node run-collections-in-directory.js \
    --folder=./nims-service/src/test/resources/functional_test/ --hostStaging=$1 && \
    node run-collections-in-directory.js \

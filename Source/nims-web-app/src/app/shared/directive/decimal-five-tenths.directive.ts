...
@Directive({
  selector: '[decimalFiveTenths]'
})
export class DecimalFiveTenthsDirective {

  // private regex: RegExp = new RegExp(/^\d*\.?\d{0,5}$/g);
  private regex: RegExp = new RegExp(/^\d{0,14}(\.\d{0,5})?$/g);
  ...
  onPaste(event: ClipboardEvent) {
    ...
    const pastedInput: string = event.clipboardData
      .getData('text/plain')
      .replace(/[^0-9.]/g, '').substring(0, 14); // get a digit-only string
